...
//
//   - tools.ts     (the agent tools that read/write/list/spawn)
...
// confirm-dialog / sandbox / size-limit guarantees in tools.ts cannot
...
const ALLOWLIST = new Set(["tools.ts", "apply.ts", "chatView.ts"]);
...
test("only the allowlisted modules in src/ touch the filesystem", () => {
  ...
  assert.deepEqual(
    ...
    "These src/ modules touch the filesystem but are not on the I/O allowlist:\n" +
      ...
      "\n\nEither move the I/O to tools.ts / apply.ts / chatView.ts, OR " +
      "expand the allowlist deliberately and explain why in ARCHITECTURE.md §3 Invariant 8."
  );
});

test("the three allowlisted modules each actually DO some I/O", () => {
